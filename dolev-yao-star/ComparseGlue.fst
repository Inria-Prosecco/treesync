...
val ps_char_is_well_formed:
  ...
  Lemma (is_well_formed_partial ps_char pre c)
  [SMTPat (is_well_formed_partial ps_char pre c)]
...
val ps_string_is_well_formed:
  ...
  Lemma (is_well_formed_partial ps_string pre s)
  [SMTPat (is_well_formed_partial ps_string pre s)]
let ps_string_is_well_formed #bytes #bl pre s =
  Seq.lemma_list_seq_bij (String.list_of_string s);
  for_allP_eq (is_well_formed_partial ps_char pre) (String.list_of_string s)
...
instance dy_bytes_like: bytes_like bytes = {
  ...
  to_from_nat = (fun sz b -> bytes_to_nat_lbytes_to_bytes b);
}
...
let bytes_pre_is_compatible_intro
  ...
  (split_proof:(b:bytes{pre b} -> i:nat{i <= length #bytes b /\ Some? (split #bytes b i)} -> Lemma (pre (fst (Some?.v (split b i))) /\ pre (snd (Some?.v (split b i))))))
