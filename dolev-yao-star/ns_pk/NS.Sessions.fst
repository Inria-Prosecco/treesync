...
let is_priv_key i secret_key p = A.is_private_dec_key ns_global_usage i secret_key (readers [P p]) "NS.key"
let is_pub_key i pub_key p = A.is_public_enc_key ns_global_usage i pub_key (readers [P p]) "NS.key"
let is_nonce (i:nat) n a b = A.is_secret ns_global_usage i n (readers [P a; P b]) (nonce_usage "NS.nonce") /\
			   (was_rand_generated_before i n (readers [P a; P b]) (nonce_usage "NS.nonce"))

type ns_nonce (i:nat) (a:principal) (b:principal) =
  s:A.secret ns_global_usage i (readers [P a; P b]) (nonce_usage "NS.nonce") {
   was_rand_generated_before i s (readers [P a; P b]) (nonce_usage "NS.nonce")
  }
type pub_key i p = A.public_enc_key ns_global_usage i (readers [P p]) "NS.key"
type priv_key i p = A.private_dec_key ns_global_usage i (readers [P p]) "NS.key"
  ...
  | Success (tn, o) ->
    ...
    | Success ("ResponderReceivedMsg3") ->
      (match split o with | Success (a, n_b) -> (match bytes_to_string a with 
					     | Success (a) -> 
					       Success (ResponderReceivedMsg3 a n_b)
...
let valid_session i p si vi st =
  ...
  | ResponderSentMsg2 a n_a n_b ->
      A.is_msg ns_global_usage i n_a (readers [P p]) /\
      A.is_secret ns_global_usage i n_b (readers [P a; P p]) (nonce_usage "NS.nonce") /\
      did_event_occur_before i p (respond a p n_a n_b) /\
      was_rand_generated_before i n_b (readers [P a; P p]) (nonce_usage "NS.nonce")
  | InitiatorSentMsg3 b n_a n_b ->
      A.is_msg ns_global_usage i n_b (readers [P p]) /\
      is_nonce i n_a p b 
...
let serialize_valid_session_st (i:nat) (p:principal) (si vi:nat) (st:session_st{valid_session i p si vi st}) : msg i (readers [V p si vi]) =
  let s2b s : msg i (readers [V p si vi]) = A.string_to_bytes #ns_global_usage #i s in
  assert (forall s. s2b s == string_to_bytes s);
   match st with
   | InitiatorSentMsg1 b n_a ->
     A.concat #ns_global_usage #i #(readers [V p si vi]) ((s2b "InitiatorSentMsg1")) (A.concat ((s2b b)) n_a)
   | ResponderSentMsg2 a n_a n_b ->
     A.can_flow_transitive i (A.get_label ns_key_usages n_a) (readers [P p]) (readers [V p si vi]);
     A.concat #ns_global_usage #i ((s2b "ResponderSentMsg2")) (A.concat ((s2b a)) (A.concat n_a n_b))
   | InitiatorSentMsg3 b n_a n_b ->
     A.can_flow_transitive i (A.get_label ns_key_usages n_b) (readers [P p]) (readers [V p si vi]);
     A.concat #ns_global_usage #i ((s2b "InitiatorSentMsg3")) (A.concat ((s2b b)) (A.concat n_a n_b))
   | ResponderReceivedMsg3 a n_b ->
     A.concat #ns_global_usage #i ((s2b "ResponderReceivedMsg3")) (A.concat ((s2b a)) n_b)
